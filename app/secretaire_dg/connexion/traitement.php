...
if (isset($_POST['email']) and !empty($_POST['email']) and filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
    ...
    $errors['email'] = '<p>Le champs email est incorrect ou vide, veuillez réessayer</p>';
}
...
if (empty($errors)) {
    ...
    $data_users = recuperer_donnees_utilisateur($data['email'], $data['mot_de_passe'],'Sécrétaire du directeur générale',1,0);
    //die(var_dump($data_users));
    //si oui, je le connecte et j'enregistre ses données dans une session.
    if (is_array(  $data_users)) {
        $_SESSION['users_SDG'] = $data_users;
        $_SESSION['data'] = "";
