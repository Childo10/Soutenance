...
if (empty($errors)) {
    ...
    $enregistrer_salle = enregistrer_salle($data['capacite'], $data['Typesal'], $data['nbmax'], $data['Nb_defectueux'], $data['Nb_dispo']);
    if ($enregistrer_salle) {
        $_SESSION['message_success_global'] = 'Salle ajoutée avec succès';
        header('location:' . CHEMIN_PROJET . 'directeur_etudes/salle/Ajout_salle');
    } else {
        $_SESSION['message_erreur_global'] = "Une erreur est survenue lors de l'enregistrement";
        header('location:' . CHEMIN_PROJET . 'directeur_etudes/salle/Ajout_salle');
    }
} else {
