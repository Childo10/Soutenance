...
if (verifier_info($_POST['email']) && filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) && check_email_exist_by_profil($_POST['email'],'Sécrétaire des études')){
 ...
 if(empty($errors)){
    ...
    if (insertion_token($id_utilisateur, 'MOT_DE_PASSE_OUBLIE', $token)) {
        ...
        include 'app/secretaire_etudes/mot_de_passe_oublie/message_mail.php'; // Inclut le fichier HTML dans le tampon
        ...
        if (send_email($data_email, $objet, $template_mail)) {
            ...
            header('location: ' . CHEMIN_PROJET. 'secretaire_etudes/mot_de_passe_oublie/');
        }
