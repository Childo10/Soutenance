...
if(empty($errors)){
    ...
    $data['profil']= "Sécrétaire des études";
    ...
    if (inscrire_utilisateur($data["nom"], $data["prenom"], $data["username"],$data["email"], $data["mot_de_passe"], $data["profil"]  )){
        ...
        if (!insertion_token($id_utilisateur, 'VALIDATION_COMPTE', $token)) {
            ...
            include 'app/secretaire_etudes/inscription/message_mail.php'; // Inclut le fichier HTML dans le tampon
            ...
            if (send_email($data["email"], $objet, $template_mail)) {
                ...
                header('location: ' . CHEMIN_PROJET. 'secretaire_etudes/inscription');
            } else {
